...
<el-row :gutter="20" class="mt-15">
  <el-col :span="6">
    ...
    <div class="grid-content" v-bind:style="gridContentHeight" v-if="billNoList.length">
      <ul class="infinite-list">
        <li v-for="(item,index) in billNoList" :key="index" class="infinite-list-item">{{item}}</li>
      </ul>
    </div>
  ...
  <el-col :span="9">
    ...
    <div class="grid-content" v-bind:style="gridContentHeight" v-if="verificationCodeList.length">
      <ul class="infinite-list">
        <li v-for="(item,index) in verificationCodeList" :key="index"  class="infinite-list-item">分配号码:{{ item.billNo }}  验证码:{{item.verificationCode}}</li>
      </ul>
    </div>
    <div class="grid-content text-c mt-15" v-bind:style="gridContentHeight" v-else>暂无数据</div>
  ...
  <el-col :span="9">
    <div class="clearfix" style="background-color: #f2f2f3;">
      <el-col :span="18"><el-input style="margin-bottom:6px;margin-top:5px;" placeholder="请输入卡号查询验证码" prefix-icon="el-icon-search" v-model="cardNumber"></el-input></el-col>
      <el-col :span="6"><el-button style="margin-bottom:6px;margin-top:9px;" size="small" type="primary" @click="tableFn">查询</el-button></el-col>    
    </div>
    <div class="grid-content" v-bind:style="gridContentHeight" v-if="tableData.length">
      <ul class="infinite-list">
        <li v-for="item in tableData" :key="item.id"  class="infinite-list-item">分配号码:{{ item.billNo }}  验证码:{{item.verifCode}}</li>
      </ul>
    </div>
...
import {getAppsList,getBillNo,getVerifCode,customerAppsList,consumeList} from "../../api/api.js";
...
export default {
    ...
    data(){
        return {
          ...
          alertShow:false,
          alertSuccess:null,
          descriptionTitle:"手机区号选择中国台湾（+886），请先在手机上点击获取验证码，再做下步操作！请在2分钟之内完成业务操作！",
          alertTitle:"",
          codeAlert:false,
    ...
    methods:{
      ...
      initEventHandle() {
        ...
        ws.onmessage = (event)=> {
          ...
          if(event.data.length>4){
            ...
            this.codeAlert=true;
            this.verificationCodeMsg="您的"+appName+"验证码是："+obj.verification_code;
          } 
      ...
      getAppList(){
          try{
            ...
            let handlerFn=async (url,paramsObj)=>{
              ...
              data.status==0?this.appList=data.data:this.$message.error(data.message);
            }
            if(userName=="admin"){
      ...
      async getNumberFn(){
        try{
          let data=await this.$axios.post(getBillNo,this._qs.stringify({
            apps:this.appVal
          }));
          if(data.status==0){
            this.alertShow=true;
          ...
          this.$message.error('网络异常，请稍候再试！');
            ...
            // this.getNumberBtn=true;
            // this.billNo=parseInt(Math.random()*1000000);
            // this.alertTitle="您的分配号码是："+this.billNo; 
            // this.billNoList.unshift( this.billNo )
      },
